...
router.post("/:mode", async (req, res, next) => {
  try {
    ...
    if (safety.crisis) {
      ...
      if (hasGemini()) {
        try {
          ...
          const recentHistory = await ChatMessage.find({ userId: req.userId, mode }).sort({ createdAt: -1 }).limit(12).lean();
    ...
    res.json({ history, crisis: safety.crisis, aiEnabled: hasGemini(), fallback: !safety.crisis && (!hasGemini() || history.at(-1)?.role !== "assistant") });
  } catch (error) { next(error); }
});
